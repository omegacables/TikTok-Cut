...
【カットの最適化ルール（重要）】
1. 切り出しは「話の頭」から。文や相槌の途中から始めない。前置きが要る時だけ最小限含める。
2. オチ・結論・感情の山（笑い／驚き／本音／煽り／感動）まで含めて終える。途中でブツ切りにしない。
3. 冒頭 1〜2 秒で掴める区間。先頭・末尾に沈黙や「えーっと」等の無駄な間を入れない。
4. 長さは {lo}〜{hi} 秒の範囲。**{lo}秒未満のクリップは絶対に作らない**。内容に応じた目安は
   「一言で笑える/驚く=18〜25秒、面白エピソード=25〜40秒、解説・ノウハウ=30〜60秒、
   感動・ストーリー=45〜90秒、深い議論・考察=60〜180秒」。短すぎる細切れにしない。1クリップ＝1つの見せ場。
5. 前後を知らない初見でも理解できる、自己完結した区間にする。
